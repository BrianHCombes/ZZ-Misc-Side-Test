...
// It returns the calculated CSS unit value based on the devices view port width or view port height.
  ...
  function groupFilter(data){
    ...
    if(axisUsed === "width"){   
      //closest = data.args[1] - lr.vpWidth;
      axisDimension = lr.vpWidth;
    } else 
    if(axisUsed === "deviceWidth"){
      //closest = data.args[1] - lr.vpHeight;
      axisDimension = window.screen.availWidth;
    } else  
    if(axisUsed === "height"){
      //closest = data.args[1] - lr.vpHeight;
      axisDimension = lr.vpHeight;
    } else
    if(axisUsed === "deviceHeight"){
      //closest = data.args[1] - lr.vpHeight;
      axisDimension = window.innerHeight;
    } else
    if(axisUsed === "ratio"){
      //closest = data.args[1] - lr.vpRatio;
      axisDimension = lr.vpRatio;
    } else
    if(axisUsed === "deviceRatio"){
      axisDimension = lr.maxVPheight/lr.maxVPwidth;
    } else
    {
...
//                      data.args[0] + "," + data.args[1] + "," + data.args[2] + "," + data.args[3] + "," + data.args[4]);
  ...
  function unitCalculator(propertyPlusUnit,vp1,v1,vp2,v2){
    ...
    if(GetSet().getAxisToUse() === "width" && GetSet().getEvalToUse() === -1){   
      return cssProperty + (slope*lr.vpWidth + yIntercept).toFixed(2) + cssUnit;
    } else 
    if(GetSet().getAxisToUse() === "deviceWidth" && GetSet().getEvalToUse() === -1){   
      return cssProperty + (slope*window.screen.availWidth + yIntercept).toFixed(2) + cssUnit;
    } else   
    if(GetSet().getAxisToUse() === "height" && GetSet().getEvalToUse() === -1){
      return cssProperty + (slope*lr.vpHeight + yIntercept).toFixed(2) + cssUnit;
    } else
    if(GetSet().getAxisToUse() === "deviceHeight" && GetSet().getEvalToUse() === -1){
      return cssProperty + (slope*window.innerHeight + yIntercept).toFixed(2) + cssUnit;
    } else
    if(GetSet().getAxisToUse() === "ratio" && GetSet().getEvalToUse() === -1){
      return cssProperty + (slope*lr.vpRatio + yIntercept).toFixed(2) + cssUnit;
    } else
    if(GetSet().getAxisToUse() === "deviceRatio" && GetSet().getEvalToUse() === -1){
      return cssProperty + (slope*(window.innerHeight/window.screen.availWidth) + yIntercept).toFixed(2) + cssUnit;
    } else
